...
public class UserService {
  ...
  private String renewToken(String token, String email) {
    ...
    redisTemplate.expire(email, 30, TimeUnit.MINUTES);//设置过期时间为30分钟
    return token; 
  }
  ...
  public User getLoginedUserByToken(String token) {
    ...
    try {
      ...
      throw new UserException(Type.USER_NOT_LOGIN,"User not login");//验证失败
    }
    String email =  map.get("email");
    Long expired = redisTemplate.getExpire(email);//返回剩余时间（以秒为单位计算）
    if (expired > 0L) {
      renewToken(token, email);
      User user = getUserByEmail(email);
      user.setToken(token);//每次鉴权都需要重新设置下token
      return user;
    }
    throw new UserException(Type.USER_NOT_LOGIN,"user not login");//token过期则验证失败
